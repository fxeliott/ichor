...
**NEXT** : ⭐ opening task of the next code session = **engine upgrade → Fable 5 effort max** (owner decision §9 ; runner model-Literal lacks `fable`, effort schema already accepts `xhigh`/`max`), then Chantier **A** (out-of-sample benchmark gate — post-Fable-5-cutoff walk-forward) per PLAN*DIRECTEUR §5, then **B** (close the learning loop). Turnkey quick-wins : collector staleness watchdog (designed 06-09) · kill the §10.2 outage class (honest exit codes + healthz runner probe) · retire the :8765 zombie runner instance. **Owner-pending (only §9.2 remains)** : S05 materials — backtest transcript + pedagogical transcript + Ichor-beta hub access. *(GAP-4 §9.1 and Couche-1 §9.3 : ✅ DECIDED 06-10, recorded verbatim in PLAN*DIRECTEUR §9.)*
