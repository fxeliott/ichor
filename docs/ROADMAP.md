...
## §1 — Current state (2026-06-12 — S05 re-fire : **Chantier E slice-1 SHIPPED+DEPLOYED+WITNESSED** — Ichor reads the chart (ADR-113) ; `main = c9e1b97`)

**The §9.2 gate fell** : the owner dropped the S05 materials (4 technical transcripts + fundamentals transcript + Ichor-beta hub) on 06-12 ~01:40 → Chantier E un-gated. **PR #234 squash-merged `c9e1b97`, deployed Hetzner (backup `ichor_api.20260612-013436`, healthz+sample 200), 5/5 prod-witnessed** (detail : [`SESSION_LOG_2026-06-12-s05-technical-methodology.md`](SESSION_LOG_2026-06-12-s05-technical-methodology.md) + [`ADR-113`](decisions/ADR-113-ichor-reads-the-chart-technical-methodology.md)) : (a) **codified SSOT [`METHODOLOGIE_TECHNIQUE_ELIOT.md`](METHODOLOGIE_TECHNIQUE_ELIOT.md)** — 10 fresh readers absorbed the raw sources (1.12M tokens), then **6 adversarial fresh verifiers refuted v1.0 → 37 findings ALL folded** (incl. a retest-polarity BLOCKER that had also reached the code) ; 16 `[TBD owner]` questions named. (b) **`services/technical_analysis.py`** (pure core, london-session pattern) + `_section_technical_methodology` in build_data_pool/build_asset_data_only : H1 candle grammar, poussée/correction + anomaly-of-role reversal read, NY origin zones N1/N2 (3-tier approach-side retest), golden zone 0.5-0.618, plongeur-wick day-open status — filling the prose slot reserved since liquidity_proxy. (c) **Pine reading aid compiled server-side (0 err), saved to the owner's TradingView account and ADDED LIVE to the GBPUSD H1 chart** next to the owner's exact codified setup (Sessions [LuxAlgo] + No gaps candles). Fresh PR reviewer : 0 BLOCKER, 2 MAJOR + 6 MINOR all folded pre-merge. 31 new tests ; suite **3316 pass** ; mypy 0 new ; no migration ; ZERO spend.

**NEXT** : ⭐ Chantier **A** (out-of-sample benchmark gate) per PLAN_DIRECTEUR §5, then **B**. S05 watch-items (named, SESSION_LOG §6) : origin-zone density (honest-absence 5/5 on the 06-11 whipsaw day — recalibrate provisional thresholds if it persists) · golden-zone minimum-magnitude filter (slice-2) · index bars J-1 lag (pre-existing data-plan limit) · **16 `[TBD owner]`** (top leverage : truncated origin-hierarchy video, RR 3 vs 2.3, re-entry window T-P vs T-C/T-G). S05 slice-2 (deferred by design) : 15m/5m triggers + 12h-13h H1 signal + DimensionVote (post-C) + canaux/switch (owner definition). Carry-overs : column-vitality probes S03 · scorer DB exit-code · NSSM admin elevation.

### Pre-2026-06-12 line preserved

## §1 — Current state (2026-06-11 late (close 06-12) — S04 re-run : geopolitics differentiation UN-GATED for real + GDELT tone column found DEAD → honest-absence guard ; `main = 1ea6eb3`)

**Two PRs squash-merged, deployed Hetzner ×2, runtime-witnessed same session** (detail : [`SESSION_LOG_2026-06-11-s04-geo-differentiation.md`](SESSION_LOG_2026-06-11-s04-geo-differentiation.md)) : (a) **PR #229 `e985a68`** — per-asset GDELT candidate pool 40→400 (data_pool + router, default-top parity CI-pinned via route introspection, deterministic `id.asc()` tiebreak). Pre-code prod witness PROVED the passive "density will fix it" watch item wrong : the 40-row whole-window most-negative pool structurally starved low-share assets (XAU matched 0/40 despite 403 gold rows/24h) ; cap-400 simulation = 5/5 assets differentiate (XAU 48, GBP 9, SPX 11). (b) **PR #230 `1ea6eb3`** — the #229 post-deploy witness exposed a LATENT hole : **`gdelt_events.tone` = 0.0 on 13,607/13,607 rows over the full 8-day retention** (ArtList JSON carries no per-article tone ; parser default silent since ingestion) → "most-negative" ranked NOTHING for ≥8 days, and the widened per-asset filter surfaced keyword noise. Column-vitality guard : ≥20 pool rows ALL tone=0.0 → ranking suspended with explicit band + `GDELT:tone` DegradedInput, AI-GPR intact, auto-disarms on real tone. **Final witness : 5/5 assets honest-suspended in prod.** Fresh-context reviewer on EACH PR (READY TO MERGE ×2, all foldable findings folded pre-merge). Chantier C slice-1 blueprint persisted to memory (`ichor_chantier_c_slice1_blueprint.md`) : fusion flow mapped, DimensionVote contract, golden-diff harness = prerequisite (does not exist), 7 CI traps, GPR doctrine web-verified (threats≠acts, 90-99th pct non-linearity, USD conditional). Tests 13+6 geo + 450 regression ; mypy 0 new ; no migration ; ZERO spend.

**Re-fire #2 (06-12 ~01:00, PR #232 `090af3c`, ADR-112)** : tone-dead follow-up ① EXECUTED — NEW `run_gdelt_tone_scorer` (local FinBERT, English titles 62%, `(p_pos−p_neg)×10` GDELT-like scale, 15-min timer + OnFailure, 48h backfill = 2,158 rows scored, no migration) + `TARIFF_SHOCK` averages SCORED rows only (reviewer MAJOR folded : 0.0-dilution would have hardened the −1.5 gate ~1.6×). **Final witness : geo 5/5 ALIVE on real negatives (guard auto-disarmed, World Bank→EUR, RICS→GBP), heatmap `mean_tone` real (US −0.66, India −1.3), tariff `avg_tone=−0.213` = alert fireable again, service `Result=success`.** Fresh reviewer APPROVE 0-blocker (8 hunt axes incl. hypertable UPDATE binding, collector race, .sh encoding).

**NEXT** : ⭐ Chantier **A** (out-of-sample benchmark gate) per PLAN_DIRECTEUR §5, then **B**. Remaining named follow-ups : ② column-vitality probes in the S03 freshness monitor (zero-variance 48h → DATA_QUALITY) = class closure ; scorer dedicated DB-transient exit code (noisy notify on Postgres blips) ; `/briefing` router tone-dead guard (now theoretical — tone lives) ; FinBERT −10 saturation granularity (ranking unaffected). Watch : GBP geo margin (widen `gbpusd_uk_economy` query if it degrades). Owner-pending : §9.2 S05 materials · NSSM admin elevation.

### Pre-2026-06-11-late line preserved

## §1 — Current state (2026-06-11 PM — S03 re-run : data-engine slice SHIPPED+DEPLOYED+WITNESSED — proactive freshness monitor, per-asset GDELT, newsletters depth, pre-announcement sentinel ; `main = 2b7f2de`)

**PR #225 squash-merged, deployed Hetzner, runtime-witnessed same day** (detail : [`SESSION_LOG_2026-06-11-s03-data-engine.md`](SESSION_LOG_2026-06-11-s03-data-engine.md) + [`ADR-111`](decisions/ADR-111-s03-proactive-data-freshness-and-collection-depth.md)) : (a) **proactive data-freshness monitor** — 29-source registry + 5-min timer, COLLECTOR_STALE/ABSENT/RSS_FEED_SILENT alerts, transition exit-2 → `ichor-notify@` ; closes the Chantier D gate "deliberately-killed collector fires an alert < 15 min" (data half — runner half was ADR-110) ; first prod tick caught 3 REAL silent feeds (boe_news 403 class + sec_press + wsj_markets) ; kill-test witnessed (polymarket stopped → alert + notify → restarted). (b) **per-asset GDELT slices** (6 queries) un-gate the S04 geopolitics differentiation (data-gated since 06-09). (c) **newsletters depth** : +10 fetch-verified feeds (BoC/SNB/BEA/StatCan/ONS/FXStreet/EIA/CNBC/OilPrice/CrisisGroup) + RSS 1.0/RDF support — 21 feeds total. (d) **pre-announcement sentinel** : ECO_EVENT_IMMINENT (critical = web-push) T-60 min before high-impact USD/EUR/GBP/CAD prints, event-cluster dedup ; "être prévenu de TOUTES les annonces" is now PROACTIVE. (e) **scenario-invalidation validation harness un-blocked** : `--dry-run` evaluates flag-OFF (read-only+rollback, notify-gated) — first run found 2 real HARD invalidations (NAS100+GBP), arming evidence accumulates in journalctl. Fresh-context verifier : 9 findings ALL folded pre-merge (incl. a true BLOCKER VARCHAR(16) overflow + hypertable full-scan probe + dry-run push leaks). Alerts import-cycle (r165) killed for good via leaf `alerts/defs.py` — CodeQL 0 open alerts. Catalog 57→61. 3,267 tests, ZERO spend, no migration.

**NEXT** : ⭐ Chantier **A** (out-of-sample benchmark gate, post-Opus-4.8-cutoff walk-forward) per PLAN_DIRECTEUR §5, then **B** (close the learning act-loop = #1 gap). Watch: per-asset GDELT density → `_section_geopolitics` `applied=True` within days · sentinel pushes on real prints · silent-feed root-cause (sec_press, wsj_markets) · scenario-invalidation validation log (≥3 sessions → owner arms the flag). Owner-pending : §9.2 S05 materials.

### Pre-2026-06-11-PM line preserved

## §1 — Current state (2026-06-11 — S02 re-run : engine **Opus 4.8 `xhigh`** LIVE + silent-outage class KILLED ; `main = 2135874`)

**PR #222 squash-merged, deployed Hetzner+Win11, runtime-witnessed** (detail : [`SESSION_LOG_2026-06-11-s02-engine-xhigh.md`](SESSION_LOG_2026-06-11-s02-engine-xhigh.md) + [`ADR-110`](decisions/ADR-110-engine-opus48-max-effort-xhigh.md)) : (a) **engine uplift** — every Couche-1 generation surface (4-pass, Pass-6, briefings, web-research, streaming_refresh) now emits `effort=xhigh` (Couche-2 stays `low`) ; timeout hierarchy moved together (runner 900 < polls 960 < walls 1200/3600/5400) ; **witnessed live** : EUR_USD card end-to-end at xhigh on prod, 449.9 s total LLM, critic approved, persisted 03:37 — batch projection ~50-70 min fits the 5400 s wall, **re-validate on the first natural batch (06-11 06:00)**. (b) **§10.2 outage class killed ×4 layers** : batch rc=2 on TOTAL failure → OnFailure notifies ; `/healthz` probes the runner (prod answers `claude_runner_reachable: true`) ; NEW `ichor-runner-health-check.timer` 5-min proactive monitor (first run exit 0) ; Win11 watchdog self-heals the WinError-2 class via the self-probing `.bat`. (c) Independent fresh-context verifier : 9 findings ALL folded pre-merge (incl. a pre-existing DSPy kwargs TypeError). **Ops carry-over** : NSSM zombie `IchorClaudeRunner` (:8765) stop+disable **needs owner admin elevation** (runner isolated on `.venv-live` meanwhile, tunnel routes :8766).

**NEXT** : ⭐ Chantier **A** (out-of-sample benchmark gate, post-Opus-4.8-cutoff walk-forward) per PLAN_DIRECTEUR §5, then **B** (close the learning act-loop = #1 gap). Watch the first xhigh batches (duration + quota, §6.1 contention rule). Owner-pending : NSSM admin elevation (30 s) + §9.2 S05 materials.

### Pre-2026-06-11 line preserved

## §1 — Current state (2026-06-10 PM — Session-01 second re-run : PLAN_DIRECTEUR **v4**, prod zones WITNESSED, same-day P0 runner outage repaired ; `main = 9a92dea` at write)

Since 06-03, four working arcs landed on main (PR #186→#217) : 06-05 S02 reliable-substrate + security deps ; 06-06 S03 honnêteté + Opus-driven web research (G1, flag ON — **ON-state now witnessed in prod DB 06-10**) ; 06-07→06-09 S04 depth — conviction **fusion LIVE** (migration `0055`, the 50/50 killed in prod), 9th dimension manip/liquidité, RVOL volume layer, per-asset geopolitics, UST 10Y rate-positioning, full CFTC trader-category breakdown, per-asset news tone, COT/ECB/BLS/EIA collectors repaired, Couche-2 free-text self-heal hardened. **Session 01 (06-10, two adversarial passes)** : AM = [`PLAN_DIRECTEUR.md`](PLAN_DIRECTEUR.md) v3 (15-agent `wf_17c396c2`, README refresh, SUPERSEDED banners, PR #218) + owner decisions recorded (PR #219) ; PM = **v4** (18-agent `wf_dc5c71eb`) — 25 claims re-refuted (0 REFUTED, 7 PARTIAL folded), 8-file spec-fidelity audit (§5ter-bis), **the 3 prod-only zones CLOSED on the Hetzner host** (12/12 flags ON ; the one un-armed flag = `scenario_invalidation_monitor_enabled` absent→OFF ; 102 timers ; Polymarket/GDELT fresh, GPR stale upstream ; alembic prod `0055`), and a **same-day P0 caught & repaired** : Win11 reboot + npm-update race → runner `[WinError 2]` → 0 cards/briefings/Couche-2 all day (masked by `SuccessExitStatus=0 1`, invisible to healthz) → runner restarted, **witnessed `couche2.run.ok` ×5 through the tunnel** (PLAN_DIRECTEUR §10.2). Learning **act-loop still OPEN = #1 gap** (locked by `test_architecture_invariants.py:133-144`).

**ENGINE REVISION 06-11 (owner, PLAN_DIRECTEUR v4.1 §9 decision 3)** : la migration Fable 5 est **ANNULÉE** — Fable 5 n'est inclus sur Pro/Max que du 9 au 22 juin (vérifié source primaire Anthropic) ; dès le 23 il consomme des usage credits au tarif API (2× Opus 4.8) = violation ZERO-spend. Moteur = **Opus 4.8 monté à l'effort `xhigh`** (schema runner l'accepte déjà — le chantier transverse rétrécit à un changement d'effort + 1 ADR). Quota-contention prouvée live 06-10 22:01 → règle : pas de grosse session interactive ±30 min des fenêtres 06/12/17/22h (PLAN §6.1).

**NEXT** : ⭐ opening task of the next code session = **engine effort uplift Opus 4.8 → `xhigh`** (call sites orchestrator/briefings/runner*client ; Couche-2 reste `low` ; ADR doctrine + témoin par surface en surveillant durée de batch et quota), then Chantier **A** (out-of-sample benchmark gate — post-Opus-4.8-cutoff walk-forward) per PLAN_DIRECTEUR §5, then **B** (close the learning loop). Turnkey quick-wins : collector staleness watchdog (designed 06-09) · kill the §10.2 outage class (honest exit codes + healthz runner probe) · retire the :8765 zombie runner instance. **Owner-pending (only §9.2 remains)** : S05 materials — backtest transcript + pedagogical transcript + Ichor-beta hub access. *(GAP-4 §9.1 et Couche-1 §9.3 : ✅ DECIDED 06-10 ; engine : ✅ REVISED 06-11 — verbatim au §9 du plan.)\_
