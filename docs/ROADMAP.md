...
**NEXT** : ⭐ Chantier **A** (out-of-sample benchmark gate) per PLAN_DIRECTEUR §5, then **B**. **Tone-dead follow-ups (named, high-leverage)** : ① real tone repair — local FinBERT on GDELT titles (news_tone_scorer pattern, Voie-D) un-gates `TARIFF_SHOCK` (alert currently CANNOT fire : `avg_tone<=-1.5` impossible) + `/heatmap` flat `mean_tone` (the ONE consumed geo endpoint) + `/briefing` router guard (reviewer #230 MAJOR, no in-repo consumer yet) ; ② column-vitality probes in the S03 freshness monitor (zero-variance 48h → DATA_QUALITY) = class closure. Watch : GBP geo margin (9 matches = thinnest ; widen `gbpusd_uk_economy` query if it degrades). Owner-pending : §9.2 S05 materials · NSSM admin elevation.
