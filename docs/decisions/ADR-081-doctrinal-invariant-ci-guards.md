...
| 4   | ADR-108         | `test_couche2_agents_reference_opus`                | Positive guard — at least one Couche-2 agent file references `"opus"` (§11 full-Opus, supersedes ADR-023 Haiku), catches accidental deletion of the model-selection logic.                                                                                                                                                                                      |
...
| 10  | ADR-017         | `test_no_actionable_order_endpoint`                 | NEGATIVE guard — walks every mounted route's `response_model` (+ nested models) and fails on order-ticket fields (`stop_loss`/`tp1`/`tp3`/`tp_extended`/`entry_zone_*`). Closes the actionable-order surface removed in `ad967ff` (`/v1/trade-plan`). Descriptive `schemas.TradePlan` (`tp_rr3`/`entry_low`/`invalidation_level`) is intentionally NOT matched. |
| 11  | ADR-017         | `test_no_order_ticket_tokens_in_python_code`        | Order-ticket identifiers (`take_profit`/`stop_loss`/`tp1`/`tp2`/`tp3`/`tp_extended`/`entry_zone`) as Python code tokens across prod sources **+ migrations**. Source-token layer that would have flagged `routers/trade_plan.py` at write time. (S02 audit 2026-06-17)                                                                                          |
