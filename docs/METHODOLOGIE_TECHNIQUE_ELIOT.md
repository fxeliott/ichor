...
**IMPLÉMENTÉ (S05 re-fire #3)** : `technical_analysis.py:compute_daily_read`
classifie la dernière bougie daily clôturée (fenêtre [22h Paris J-1, 22h Paris J))
via le SSOT `classify_daily_candle` (avec la bougie précédente pour l'avalement),
lit le **côté de rejet** (mèche dominante > corps) et **pilote l'attente de la
mèche du plongeur** (§5.2) : clôture en forte poussée → mèche non requise ;
clôture en rejet → mèche attendue (continuation du rejet avant le sens final).
Close 22h Paris = PROVISIONAL (bord 22h/23h DST + No-Gap §13.10). Rendu
descriptif (ADR-017), absence honnête si couverture intraday insuffisante.
...
| **Lecture Daily (§5.3)** : dernière bougie daily clôturée (22h Paris, §13.15), pleine/incertitude + côté de rejet + attente mèche du plongeur | `technical_analysis.py:compute_daily_read` (réutilise `daily_candle_classifier`) + `DailyRead` + rendu                           | **IMPLÉMENTÉ S05 re-fire #3** — close 22h PROVISIONAL (No-Gap §13.10)              |
...
| Retest **3 sous-zones S/R** (paliers 1/3 et 2/3 internes)                                                                                     | `technical_analysis.py:OriginZone.sub_zone_dividers` + rendu                                                                     | **IMPLÉMENTÉ S05 re-fire #3** (§13.5) — retest band binaire conservé en complément |
...
| Indices de retournement gradués + état confirmation ; intention de bougie (§5.1)                                                              | `technical_analysis.py` : `read_trend` (`indices_retournement` + `confirmed_retournement`) + `candle_intention` + rendu          | **IMPLÉMENTÉ S05 re-fire #3**                                                      |
...
5. 🟡 **PARTIEL (S05)** : tracé confirmé (clôture du corps + extension mèche,
   ...
   divisée en 3 sous-zones S/R **CODÉE** (`OriginZone.sub_zone_dividers`, paliers
   1/3 et 2/3, rendus — S05 re-fire #3). STILL_OPEN : largeur maximale CHIFFRÉE +
