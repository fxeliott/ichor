...
| **S07** | Learning & self-improvement                      | **52%**  | Measurement is LIVE (Brier/Vovk/ADWIN/reconciliation crons) but the **act loop is OPEN**: `pocket_skill_reader` is dead code (0 callers from generation, test-locked). _Corrected v4:_ the W116c addendum flag (`w116c_llm_addendum_enabled`) **is ON in prod** (witnessed 06-10, §10.1, with `pass3_addenda_injection_enabled`) — the addendum side-channel is armed; what stays unwired is the core skill re-injection (Vovk weights → confluence). Ichor logs, doesn't act on measured skill. **This is the #1 gap.** |
