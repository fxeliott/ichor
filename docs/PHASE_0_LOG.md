...
| 11 | Redis Streams setup + producers asyncio | 🟢 | Redis Pub/Sub channels wired in `apps/api/src/ichor_api/routers/ws.py`. **Two no-API-key collectors LIVE on Hetzner** : `apps/api/src/ichor_api/collectors/{rss.py,polymarket.py}` — Alembic migration `0002_collector_tables.py` adds `news_items` + `polymarket_snapshots` TimescaleDB hypertables. systemd timers `ichor-collector-rss.timer` (every 15 min) + `ichor-collector-polymarket.timer` (every 5 min) enabled. **160 real news items + 1 Polymarket snapshot persisted**. New `/v1/news` API endpoint + `/news` UI page. Tests : 35/35 green (RSS 14 + Polymarket 8 + Crisis Mode 13) |
