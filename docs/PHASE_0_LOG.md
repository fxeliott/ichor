...
| 10 | wal-g WAL streaming Postgres → R2 EU bucket + 1er test restauration | 🟢 | wal-g 3.0.8 LIVE: basebackup `base_000000010000000000000006` written to R2 `ichor-walg-eu/postgres/basebackups_005/`, 3 WAL files archived to `wal_005/`. systemd timer `walg-basebackup.timer` enabled. **Restore drill** runbook + script committed: [RUNBOOK-010](runbooks/RUNBOOK-010-walg-restore-drill.md) + `scripts/hetzner/walg-restore-drill.sh` (in-place dry-run validates structure without touching production PGDATA). Quarterly cadence with records under `docs/dr-tests/` |
| 11 | Redis Streams setup + producers asyncio | 🟢 | Redis Pub/Sub channels wired in `apps/api/src/ichor_api/routers/ws.py`. **Three no-API-key collectors LIVE on Hetzner** : `apps/api/src/ichor_api/collectors/{rss.py,polymarket.py,market_data.py}` — Alembic migrations `0002_collector_tables.py` (news + polymarket) + `0003_market_data.py` + `0004_backtest_runs.py`. systemd timers : `ichor-collector-rss.timer` (every 15 min), `ichor-collector-polymarket.timer` (every 5 min), `ichor-collector-market_data.timer` (daily 23:10 Paris). **160+ news, 7+ polymarket snapshots, 20 556 daily OHLCV bars persisted**. APIs `/v1/news`, `/v1/market/{asset}`, `/v1/predictions`, `/v1/backtests` LIVE. UI pages `/news`, `/portfolio`, `/backtests` LIVE. Tests : 46/46 green api + 81 across new packages. |
| 12 | ML stack install (hmmlearn + dtaidistance + river + NumPyro + arch + ...) | 🟢 | Scaffolds for HMM/HAR-RV/VPIN/aggregator. **First model trained end-to-end** : `packages/ml/src/ichor_ml/training/{features,lightgbm_bias}.py`, walk-forward backtest harness (`packages/backtest/`), risk engine + kill switch (`packages/risk/`), paper trading layer (`packages/trading/`, ADR-016 paper-only contract). LIVE on Hetzner: 2 036 predictions persisted to `predictions_audit` for `lightgbm-bias-eur_usd-1d-v0-20260503` ; 1 backtest_run persisted to `backtest_runs` (Brier 0.446, hit rate 49.85 %, total return +1.03 % vs B&H +2.27 % — coin-flip honest). 81 tests across the 4 new packages (17+28+25+11). |
| 13 | NLP self-host : FOMC-RoBERTa + FinBERT-tone HuggingFace download | 🟢 | FinBERT-tone wrapper live in `packages/ml/src/ichor_ml/nlp/finbert_tone.py`. **FOMC-RoBERTa** wrapper at `packages/ml/src/ichor_ml/nlp/fomc_roberta.py` (lazy load 1.4 GB, 3-class hawkish/dovish/neutral, chunk aggregation tested 7/7 green) |
...
| 16 | Tableau model_registry.yaml + 1 model card par modèle | 🟢 | `packages/ml/model_registry.yaml` v1.1.0 — 14 entries (8 scaffolded + 6 planned). **12 model cards** in `packages/ml/model_cards/` (Mitchell 2019 format): hmm-regime-3state, adwin-concept-drift, har-rv, sabr-svi, vpin, dtw-analogues, finbert-tone, fomc-roberta, lightgbm-bias, xgboost-bias, random-forest-bias, logistic-bias, bayesian-numpyro-bias, mlp-torch-bias |
...
| # | Item | Status | Notes |
|---|------|--------|-------|
| 25 | Next.js 15 minimal Cloudflare Pages deploy `app.ichor.app` | 🟡 | `apps/web` 9 routes LIVE building cleanly (102 KB shared first-load JS): `/`, `/briefings/[id]`, `/alerts`, `/assets`, `/assets/[code]`, `/news`, `/portfolio`, `/backtests`, `/not-found`. Cloudflare Pages deploy itself awaits Eliot's `wrangler login` |
| 26 | Service worker PWA + VAPID push test (iOS Eliot + Android) | 🟡 | `apps/web/public/sw.js` registered via `<ServiceWorkerRegister>` in production layout. Cache-first static + network-first HTML + push handler stub. Manifest `/manifest.webmanifest` + icon SVG live. VAPID push activation = Phase 1 (needs server-side keys + push subscription endpoint) |
| 27 | 12 composants design system canon | 🟢 | **13/12 components live** in `packages/ui/src/components/`: BiasBar, AssetCard (rewritten as polymorphic article/button), RegimeIndicator, DisclaimerBanner, ConfidenceMeter, SourceBadge, AlertChip, BriefingHeader, EmptyState, AudioPlayer, **ChartCard** (SVG sparkline + reference line + band overlay), **DrillDownButton** (loading + disabled states + ghost variant), **Timeline + TimelineMarker** (alerts/briefings/regime overlay) |
| 28 | Logo + palette + 3 mockups asset cards via skill `canvas-design` | 🟢 | Wordmark logo at `apps/web/public/logo.svg` + smaller icon at `apps/web/public/icon.svg`. Inline mini-logo in nav header (`apps/web/app/layout.tsx`). Full design-token palette in `apps/web/app/globals.css` (surfaces / text / borders / bias-direction / severity / accent + Inter + JetBrains Mono + cubic-bezier-out + reduce-motion + focus-visible). Mockups = the 3 LIVE asset cards in `/assets` (FX majors / metals / indices grouped sections) |
