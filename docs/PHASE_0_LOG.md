...
| 11 | Redis Streams setup + producers asyncio | 🟡 | Redis Pub/Sub channels wired in `apps/api/src/ichor_api/routers/ws.py`. **Two no-API-key collectors landed** : `apps/api/src/ichor_api/collectors/{rss.py,polymarket.py}` — RSS pulls 6 finance feeds (Fed, ECB, BoE, Treasury, SEC, BBC), Polymarket public gamma API for prediction-market odds. Both have pure-parsing tests (22 passed) |
