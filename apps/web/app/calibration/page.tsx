...
import { ApiError, getCalibrationByAsset, getCalibrationByRegime, getCalibrationOverall } from "../../lib/api";
    ...
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
      <header>
        ...
        <p className="text-sm text-neutral-400 mt-1 max-w-2xl">
          Track-record honnête du pipeline 4-pass. Score de Brier moyen sur les{" "}
          {overall.n_cards} cartes de session déjà réconciliées (cf.
          <em> services/brier.py</em>). Plus c&apos;est bas, mieux c&apos;est —
          0 = parfait, 0,25 = pile/face, 1 = anti-parfait. Le score skill
          compare à la baseline 50/50 : positif = on bat le hasard.
        </p>
      </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,400px)_1fr] gap-6 items-start">
          <div>
            ...
            <p className="mt-2 text-[11px] text-neutral-500 leading-snug">
              Diagonale verte = parfaitement calibré. Bulles bleues = la
              fréquence réalisée dépasse la prédiction (sous-confiant).
              Bulles rouges = on prédit plus que ce qui s&apos;est passé
              (sur-confiant). Taille bulle = nombre de cartes dans le bin.
            </p>
          </div>

          <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
            <dt className="text-neutral-400">Cartes réconciliées</dt>
            <dd className="text-neutral-100 font-semibold">{overall.n_cards}</dd>
            <dt className="text-neutral-400">Brier moyen</dt>
            <dd className="text-neutral-100 font-semibold font-mono">
              {overall.mean_brier.toFixed(4)}
            </dd>
            <dt className="text-neutral-400">Skill vs 50/50</dt>
            <dd>
              <span
                className={[
            ...
            <dt className="text-neutral-400">Hits / misses</dt>
            <dd className="text-neutral-100 font-mono">
              <span className="text-emerald-300">{overall.hits}</span>
              {" / "}
              <span className="text-red-300">{overall.misses}</span>
              <span className="text-neutral-500 ml-2">
                ({overall.n_cards > 0
                  ? fmtPct(overall.hits / overall.n_cards)
                  : "—"})
              </span>
            </dd>
          </dl>
                ...
                {byAsset.groups.map((g) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-right font-mono text-neutral-300">
                      {g.summary.n_cards > 0
                        ? fmtPct(g.summary.hits / g.summary.n_cards, 0)
                        : "—"}
                    </td>
                  </tr>
                ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {byRegime.groups.map((g) => (
              ...
              >
                ...
                <p className="mt-1 text-lg font-semibold text-neutral-100 font-mono">
