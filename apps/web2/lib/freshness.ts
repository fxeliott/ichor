...
// ───────────────────────────────────────────────────────────────────────
// WEEKEND/HOLIDAY-AWARE "temps réel" SUBTITLE GATE
// ───────────────────────────────────────────────────────────────────────
//
// The `deriveFreshness` gate above stops a STALE card from claiming "temps
// réel". But a FRESH card generated by the weekend cron still claimed
// "Lecture en temps réel · recalibrée chaque session" while the markets are
// CLOSED — the "temps réel over marchés fermés" contradiction Eliot flagged
// 2026-05-29. This composes the card-freshness with the market-open state so
// the live wording is suppressed when the market is closed, even for a fresh
// card. `marketClosed` is derived by the caller from `getNyWindowStatus`
// (weekend → all assets ; equity holiday → SPX/NAS only ; FX/XAU keep trading
// on US holidays so are NOT marked closed) — kept as a plain boolean here so
// this module stays decoupled from `lib/nyWindow`. Precedence :
//   absent → market_closed → stale → live.
// Pure ; ADR-017 (re-expresses provenance/market-state as context, never a
// signal) ; zero LLM (Voie D).

export type FreshnessSubtitleVariant = "live" | "stale" | "absent" | "market_closed";

export function freshnessSubtitleVariant(
  cardGeneratedAt: string | null,
  marketClosed: boolean,
  now: Date = new Date(),
): { variant: FreshnessSubtitleVariant; ageLabel: string } {
  const f = deriveFreshness(cardGeneratedAt, now);
  // Absence of a card wins over everything — never imply a reading exists.
  if (f.state === "absent") return { variant: "absent", ageLabel: "" };
  // Market closed dominates the freshness claim : a fresh weekend card is
  // still "the last session's read, markets closed", not "temps réel".
  if (marketClosed) return { variant: "market_closed", ageLabel: f.ageLabel };
  if (f.state === "stale") return { variant: "stale", ageLabel: f.ageLabel };
  return { variant: "live", ageLabel: f.ageLabel };
}
