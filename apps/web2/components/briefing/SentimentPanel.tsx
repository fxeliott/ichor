...
function SplitBar({ longPct, shortPct }: { longPct: number; shortPct: number }) {
  return (
    <div className="flex h-2.5 w-full overflow-hidden rounded-full bg-[var(--color-bg-base)]">
      <m.div
        initial={{ width: 0 }}
...
export function SentimentPanel({ entries, asset }: SentimentPanelProps) {
  ...
  return (
    ...
    >
      ...
      {focus && (
        <div className="border-b border-[var(--color-border-subtle)] bg-[var(--color-bg-elevated)]/30 px-6 py-5">
          ...
          {others.map((e, i) => (
            ...
            >
              <div className="flex items-center gap-4">
                <span className="w-16 shrink-0 font-mono text-sm text-[var(--color-text-secondary)]">
                  {e.pair}
                </span>
                <div className="flex-1">
                  <SplitBar longPct={e.long_pct} shortPct={e.short_pct} />
                </div>
                <span className="w-28 shrink-0 text-right font-mono text-xs tabular-nums text-[var(--color-text-muted)]">
