...
Pure-Python skeleton ratified by [ADR-085](../../../../../docs/decisions/ADR-085-pass-6-scenario-decompose-taxonomy.md).
This module exposes :
...
Does NOT include : (a) the LLM prompt itself — that lives in
`packages/ichor_brain/.../passes/scenarios.py` when W105c lands ;
(b) the realized-outcome reconciler — that lives in
`apps/api/.../cli/reconcile_outcomes.py` when W108 lands ;
(c) the calibration bins computation — that lives in
`apps/api/.../services/scenario_calibration.py` when W105b lands.
...
# Pass-6 (LLM Claude Haiku low) must never reference a trade instruction.
# Word-boundary case-insensitive. Catches: `BUY`, `SELL`, `TP`, `SL`,
# `long entry`, `short entry`. Tested via test_invariants_ichor.py +
# test_scenarios.py at construction time.
...
def bucket_for_zscore(z: float) -> BucketLabel:
    ...
    realized-outcome reconciler (W108) to compute `realized_scenario_bucket`
