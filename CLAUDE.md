...
> Last sync: 2026-05-12 (post-W105a+c+d + Security CVE batch — autonomous session: ScenariosPass LLM prompt landed (`packages/ichor_brain/passes/scenarios.py`), migration 0039 ships `session_card_audit.scenarios JSONB + realized_scenario_bucket` + new table `scenario_calibration_bins`, orchestrator wires Pass-6 behind `enable_scenarios=False` default (zero-diff legacy), `SessionCard.scenarios` field added, persistence `to_audit_row` extended. Security audit closed MinIO `RELEASE.2025-01-20 → 2025-10-15` (CVE-2025-62506 IAM privesc HIGH) + ClickHouse `24.12-alpine → 24.12.5.65-alpine` (CVE-2025-1385 MED pin). XAU framework cite WGC quarterly DROPPED — rephrasé `passes/asset.py:122` vers CFTC TFF + COMEX inventory. 150 tests pass (70 api + 80 brain). Baseline post-W105d. Remaining sub-waves : W105b calibration service (z-score thresholds per asset rolling 252d EWMA λ=0.94), W105g reconciler `realized_scenario_bucket` + Brier multi-class K=7 + 3 baselines (uniform 6/49, climatology empirique, persistence), W105h /calibration page extension 7-bucket reliability per asset. W102 CF Access still pending Eliot 15 min dashboard).
