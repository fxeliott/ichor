...
>
> **Last sync: 2026-05-14 13:20 CEST — ROUND-43 SHIPPED : GAP-A continuation 3/5 closed (SPX500_USD `_section_spx_specific` 3-driver VIX-funding-sentiment triangle = VIX term-structure VIXCLS/VXVCLS daily + NFCI weekly + NFIB SBOI monthly)** : main HEAD = `9a59c9e` (PR #130 r43 omnibus). 1 PR merged r43. New `_section_spx_specific` invokes Brunnermeier-Pedersen 2009 funding-liquidity-spiral framework (SPX-equity analogue of Stephen Jen USD broken-smile, applied to vol-funding doom loop). R24 SUBSET-not-SUPERSET cleared (daily + weekly + monthly with frequency mismatch warning, BTP r34 precedent). Symmetric language + Tetlock invalidation on ALL 3 drivers. ADR-089 SPY proxy caveat annotated inline. AND/OR asymmetry rationale documented (tail-stress propagation faster than calm-regime accumulation). 2 ichor-trader YELLOW applied pre-merge (AND/OR doc + Stephen Jen → Brunnermeier-Pedersen attribution correction). 2 asset pairs still pending GAP-A : JPY (R24 DEFER, GAP-D blocker), AUD (R24 DEFER, commodity surface absent). Frontend gel rounds 13-43 (31 rounds zero `apps/web2`). ZERO Anthropic API spend.
>
