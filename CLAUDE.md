...
>
> **Last sync: 2026-05-14 08:10 CEST — ROUND-37 SHIPPED : 3 W90 guards for gepa_optimizer (ADR-091 contractual surface CI-pinned) + frequency-aware `_latest_fred` via `_FRED_SERIES_MAX_AGE_DAYS` registry (r35-audit-gap STRUCTURAL closure) + bundesbank CSV fixture r32c-followup (5 silent CI failures on main resolved)** : main HEAD = `def82d8` (PR #118 round-37 W90+FRED-freq+bundesbank-fix) ; 1 omnibus PR merged round-37 (3 commits squashed : W90 guards + FRED registry + bundesbank fixtures) ; 13 new FRED registry tests + 3 new W90 guards + 5 bundesbank tests resurrected = 21 new green tests ; r35 PR #113 explicit-override hot-fix replaced by structural registry resolution (caller no longer needs `max_age_days=120` kwarg — `_FRED_SERIES_MAX_AGE_DAYS["IRLTLT01ITM156N"]=120` resolves automatically) ; ADR-091 contractual surface (`_GEPA_BUDGET_HARD_CAP=100` + `count_violations` import + `compute_fitness_with_hard_zero` returns `-inf` not soft-lambda) now CI-pinned via source-inspection tokenize-based regex tests ; main branch CI green (was failing silently since r32c on 5 bundesbank tests with comma-vs-semicolon fixture mismatch) ; frontend gel intact rounds 13-37 (25 rounds zero `apps/web2`).
>
